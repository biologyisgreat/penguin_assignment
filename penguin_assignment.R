...
#separated for the different species with the linear models plotted
plot_bill_figure <- function(penguins_bills){
  penguins_bills %>% 
    ...
    theme_bw()+
    scale_shape_discrete(labels=c("Adelie", "Chinstrap", "Gentoo"))+     
    scale_colour_manual(values = c("pink2","orchid4","skyblue4"), labels=c("Adelie", "Chinstrap", "Gentoo"))+     
...
save_bill_plot_png(penguins_bills, "figures/fig01.png", size=20, size1= 15, res=600, scaling=1)


save_bill_plot_svg(penguins_clean, "figures/fig01_vector.svg", 
                      size = 20, size1= 15, scaling = 1)
